...
namespace camodocal
{
    ...
    Chessboard::findCorners(bool useOpenCV)
    {
        ...
        if (mCornersFound)
        {
            ...
            {
                float x = mCorners[i].x; // 当前点的 x 坐标
                float y = mCorners[i].y; // 当前点的 y 坐标
                // 可以在这里对 x 和 y 进行其他操作
            }

            // todo2：范围 for 循环（C++11以上版本）
            // for (元素类型 变量名 : 容器名)
            for (const cv::Point2f &it : mCorners)
            {
                float x = it.x; // ps：it 已经代表了每个元素了，所以不能用 it[0]、it[1] 了
                float y = it.y;
            } */
        }
    }
    ...
    {
        // ps1：调用OpenCV的标准方法 cv::findChessboardCorners()
        if (useOpenCV)
        {
            return cv::findChessboardCorners(image, patternSize, corners, flags);
        }
        // ps2：调用自定义的改进版方法 findChessboardCornersImproved()
        else
        {
    ...
    // notice： 从黑白棋盘标定板中提取内角点 --- 自定义的改进版方法
                                                   ...
                                                   int flags)
    {
        ...
        // 1、如果图像深度不是 CV_8U，则不符合要求，因为许多图像处理算法（如角点检测）都期望输入为8位灰度或彩色图像。
        // 2、棋盘角点检测通常需要1通道（灰度图）或3通道（彩色图），2通道图像并不常见，且在这里不被支持。
        if (image.depth() != CV_8U || image.channels() == 2)
        {
            return false;
        }

        // ps：检查棋盘格尺寸是否在合理范围内（2 到 127 之间），注意：指的是棋盘格【内部角点的数量】，即每一行和每一列中角点的数量，而不是每个棋盘格的物理长度或宽度。
        // ps：最好选用行数和列数不相等的棋盘格，这样在算法中容易识别棋盘格的行数和列数！
        ...
        // Image histogram normalization and BGR to Grayscale image conversion (if applicable)
        // MARTIN: Set to "false"
        // ps：检查是否需要将图像转换为灰度图或进行直方图均衡化。
        if (image.channels() != 1 || (flags & cv::CALIB_CB_NORMALIZE_IMAGE))
        {
            ...
            {
                cv::equalizeHist(image, norm_img); // ps：对图像image进行直方图均衡化，以增强对比度，使棋盘格的角点更容易被检测。
                img = norm_img;
            }
        }

        if (flags & cv::CALIB_CB_FAST_CHECK) // ps：进行快速检查，以快速判断图像中是否可能存在棋盘格。
        {
            // ps：检查图像中是否有可能存在给定 patternSize 大小的棋盘格。如果没有棋盘格的可能性，直接返回false，跳过进一步的处理。
        ...
        // note PART 1: FIND LARGEST PATTERN ---寻找最大的棋盘格图案
        //-----------------------------------------------------------------------
        // ps：该过程通过【膨胀背景】并应用【Canny边缘检测器】来完成。
        // Checker patterns are tried to be found by dilating the background and then applying a canny edge finder on the closed contours (checkers).
        // Try one dilation run, but if the pattern is not found, repeat until max_dilations is reached.

        /*  步骤分析：
         1、背景膨胀：
            代码中提到通过膨胀背景来帮助检测棋盘格图案。膨胀操作（dilation）会将图像中物体的边界向外扩展，从而使背景区域更大、图案更清晰。此操作有助于填充角点之间的间隙，避免断裂的边缘影响检测效果。
         2、闭合轮廓：
            膨胀后，图案的边缘会变得更为清晰，进而形成封闭的轮廓。这种封闭轮廓便于下一步的边缘检测。
         3、Canny边缘检测：
            Canny边缘检测是一种经典的边缘检测算法，能够有效识别图像中的轮廓。通过在封闭轮廓上应用Canny，可以提取出棋盘格的外边缘，使每个格子都清晰可见。
         4、迭代膨胀：
            max_dilations 控制了膨胀的最大次数。代码中将先尝试一次膨胀，如果未找到图案，将继续重复膨胀操作，直到达到最大迭代次数。通过逐步增加膨胀次数，可以在不同尺度下尝试检测，以找到最佳的棋盘格图案。
         */

        int prevSqrSize = 0; // 存储上一次检测到的棋盘格单个格子的大小。初始化为 0 表示初始状态下没有先前的格子大小信息。
        bool found = false;  // 表示是否成功检测到棋盘格角点。初始化为 false 表示初始状态下未找到棋盘格。

        // ChessboardCornerPtr 是指向角点对象的智能指针（或者自定义的指针类型），代表棋盘格的角点。每个角点对象通常包含坐标信息（如x和y位置），有助于构建完整的棋盘格图案。
        std::vector<ChessboardCornerPtr> outputCorners; // 存储检测到的棋盘格角点坐标。

        /*
            通过处理图像，将其转换为二值图，接着通过膨胀操作增强特征，以便更好地检测棋盘格图案。
            通过不断尝试不同的参数设置（如膨胀次数和自适应阈值），尽量找到准确的角点和棋盘格的布局。
            最后，代码还会检查找到的结果是否符合预期，并调整参数以提高检测的准确性。
         */
        for (int k = 0; k < 6; ++k)
        {
            for (int dilations = minDilations; dilations <= maxDilations; ++dilations)
            {
                ...
                // convert the input grayscale image to binary (black-n-white) --- 将输入的灰度图像转换为二值图像（黑白图）
                if (flags & cv::CALIB_CB_ADAPTIVE_THRESH)
                {
                    ...
                    // convert to binary --- 自适应阈值转换为二值图像
                    cv::adaptiveThreshold(img, thresh_img, 255, cv::ADAPTIVE_THRESH_MEAN_C, cv::THRESH_BINARY, blockSize, (k / 2) * 5); // 将图像转换为二值图像，适用于自适应阈值。
                }
                else
                {
                    // empiric threshold level --- 经验阈值
                ...
                // 定义交叉和矩形的结构元素，以用于膨胀操作。
                // cv::Mat kernel1 = cv::getStructuringElement(CV_SHAPE_CROSS, cv::Size(3, 3), cv::Point(1, 1));
                cv::Mat kernel1 = cv::getStructuringElement(cv::MORPH_CROSS, cv::Size(3, 3), cv::Point(1, 1)); // 基本都是把 CV_ 开头的常量换成了 cv:: 开头
                ...
                // ps：开始执行膨胀操作
                // 根据当前的膨胀次数 dilations，依次应用不同的膨胀操作，增强图像中的特征。
                if (dilations >= 1)
                ...
                // 在膨胀后的图像上应用Canny边缘检测
                // cv::Canny(dilated_image, edge_image, threshold1, threshold2);
                ...
                // 在膨胀后的图像上绘制一个白色矩形，以避免在边缘截断的轮廓
                // 在二值图像的边缘绘制一个白色矩形，以确保 findContours 可以检测到边缘的轮廓，避免边缘被裁剪。
                ...
                generateQuads(quads, thresh_img, flags, dilations, true); // 生成四边形，存储在 quads 中。
                if (quads.empty())
                {
                    continue; // 如果没有找到四边形，继续下一次循环
                }

                // The following function finds and assigns neighbor quads to every
                // quadrangle in the immediate vicinity fulfilling certain
                // prerequisites
                findQuadNeighbors(quads, dilations); // 查找相邻的四边形，确保可以找到棋盘格的邻接关系。
                ...
                for (int group_idx = 0;; ++group_idx)
                {
                    ...
                    // 找出连接的四边形，并为每个组分配一个唯一的标识符。
                    findConnectedQuads(quads, quadGroup, group_idx, dilations);

                    if (quadGroup.empty())
                    {
                        break; // 如果没有找到连接的四边形，跳出循环
                    }

                    // 清理超出预期数量的四边形，保证检测结果的准确性。
                    ...
                    // ps：计算平均距离并更新 prevSqrSize，以便为下一次迭代提供更合适的参数。
                    float sumDist = 0;
                    int total = 0;
        ...
        {
            corners.clear(); // 清空 corners 列表准备好容器
            ...
            // ps：设置亚像素精确化的终止条件 --- 更精确地确定内角点坐标
            // ps：EPS 和 MAX_ITER 都是终止条件类型的标志，通常用于控制迭代算法的停止标准
            // EPS：表示误差限条件（epsilon），即算法在每一步的误差变化小于指定阈值时停止。
            // MAX_ITER：表示迭代次数条件，即算法达到指定的最大迭代次数后停止。
            // ps： 当把这两个条件相加（cv::TermCriteria::EPS + cv::TermCriteria::MAX_ITER）时，表示两种条件都可以触发停止：当达到误差限或最大迭代次数时，算法都会终止。
            // cv::TermCriteria(cv::TermCriteria::EPS + cv::TermCriteria::MAX_ITER, 30, 0.1)--- 最大迭代次数为 100 次或误差变化小于 0.1 时停止，即：要么处理到 30 次迭代，要么误差小于 0.1，就停下来。
    ...
    //===========================================================================
    // note：从输入图像中检测轮廓，识别并提取符合条件的四边形（即棋盘格单元格）（quads）
    void Chessboard::generateQuads(std::vector<ChessboardQuadPtr> &quads, // 存储检测到的四边形的容器。
                                   cv::Mat &image,                        // 输入图像，通常是经过预处理的二值图像。
                                   int flags,                             // 标志位，用于控制检测过程中的不同选项。
                                   int dilation,                          // 膨胀参数，可能用于图像处理。
                                   bool firstRun)                         // 指示当前是否为第一次运行，用于控制轮廓的检测策略。
    {
        // Empirical lower bound for the size of allowable quadrangles.
        // MARTIN, modified: Added "*0.1" in order to find smaller quads.
        // 计算最小四边形大小：通过图像的宽度和高度计算出最小的允许四边形大小。此值用于过滤掉面积过小的轮廓。
        ...
        // ps：使用 cv::findContours() 函数查找输入图像中的轮廓，并将轮廓及其层级结构存储在 contours 和 hierarchy 中。
        ...
        for (size_t i = 0; i < contours.size(); ++i)
        {
            ...
            {
                // 使用cv::approxPolyDP() 来逼近轮廓，以确定其形状。
        ...
        // Allocate quad & corner buffers --- 创建四边形结构
        // 通过存储在 quadContours 中的有效四边形，分配 quads 中的四边形结构，并填充每个四边形的角点信息。
